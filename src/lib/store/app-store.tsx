...
const INIT_VISITORS: Visitor[] = [
  { id: "v1", name: "田中 太郎", rank: "gold", status: "active", table: "T1", amount: 1500, time: "18:30" },
  { id: "v2", name: "鈴木 花子", rank: "vip", status: "active", table: "T1", amount: 800, time: "19:15" },
  { id: "v3", name: "佐藤 健一", rank: "silver", status: "active", table: "T3", amount: 0, time: "20:00" },
  { id: "v4", name: "渡辺 優子", rank: "gold", status: "unassigned", table: "", amount: 0, time: "20:30" },
  { id: "v5", name: "山本 翔太", rank: "silver", status: "unassigned", table: "", amount: 0, time: "20:45" },
];

const INIT_STAFF: StaffMember[] = [
  { id: "s1", name: "山田 太郎", role: "ディーラー", status: "working", clockIn: "18:00" },
  { id: "s2", name: "鈴木 一郎", role: "ディーラー", status: "on_break", clockIn: "18:00" },
...
const INIT_TIMELINE: TimelineEvent[] = [
  { id: "tl1", time: "21:30", type: "入店", name: "田中 太郎", rank: "gold" },
  { id: "tl2", time: "21:15", type: "入店", name: "佐藤 花子", rank: "vip" },
  { id: "tl3", time: "21:00", type: "出勤", name: "山田 太郎", detail: "ディーラー" },
  { id: "tl4", time: "20:45", type: "入店", name: "山本 翔太", rank: "silver" },
  { id: "tl5", time: "20:30", type: "入店", name: "渡辺 優子", rank: "gold" },
  { id: "tl6", time: "20:15", type: "退勤", name: "伊藤 美咲", detail: "フロア" },
  { id: "tl7", time: "20:00", type: "イベント", name: "VIPナイト", detail: "開始" },
  { id: "tl8", time: "19:15", type: "入店", name: "鈴木 花子", rank: "vip" },
  { id: "tl9", time: "18:30", type: "入店", name: "田中 太郎", rank: "gold" },
  { id: "tl10", time: "18:00", type: "出勤", name: "鈴木 一郎", detail: "ディーラー" },
  { id: "tl11", time: "18:00", type: "出勤", name: "佐藤 花", detail: "フロア" },
  { id: "tl12", time: "17:30", type: "出勤", name: "高橋 健", detail: "ディーラー" },
];

const INIT_EVENTS: EventItem[] = [
  { id: "e1", title: "VIPナイト", time: "20:00-24:00", status: "進行中", participants: 12 },
  { id: "e2", title: "新作カクテル試飲会", time: "22:00-23:00", status: "準備中", participants: 8 },
];
...
export function AppStoreProvider({ children }: { children: ReactNode }) {
  ...
  const totalSales = visitors.filter(v => v.status === "settled").reduce((s, v) => s + v.amount, 0)
    + visitors.filter(v => v.status !== "settled").reduce((s, v) => s + v.amount, 0);
  ...
  const orderCount = timeline.filter(t => t.type === "注文" || t.type === "精算").length;
  ...
  const kpis: KPIs = {
    ...
    onDuty,
    sales: totalSales > 0 ? totalSales : 185000,
    avgSpend: activeVisitors.length > 0 ? Math.round((totalSales > 0 ? totalSales : 185000) / activeVisitors.length) : 8043,
    orders: orderCount > 0 ? orderCount : 47,
    targetSales: 250000,
  };
  ...
  const addVisitor = useCallback((name: string, rank: Rank) => {
    const time = now();
    const id = `v${Date.now()}`;
    setVisitors(prev => [{ id, name, rank, status: "unassigned", table: "", amount: 0, time }, ...prev]);
  ...
  const settleVisitor = useCallback((id: string) => {
    setVisitors(prev => {
      ...
      if (v) {
        setTimeline(tl => [{ id: `tl${Date.now()}`, time: now(), type: "精算", name: v.name, amount: v.amount, rank: v.rank }, ...tl]);
      }
