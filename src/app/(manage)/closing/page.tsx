...
export default function ClosingPage() {
  ...
  if (result?.success) {
    return (
      <div className="max-w-lg mx-auto mt-12 text-center">
        <div className="bg-bg-white border border-border rounded-[var(--radius-lg)] p-8">
          ...
          <button
            ...
            className="px-3 py-[7px] text-[13px] font-medium text-accent hover:bg-accent-light rounded-[var(--radius)] transition-colors"
          >
            別の日付で締め処理
          </button>
        </div>
  ...
  return (
    <div className="space-y-4">
      ...
      {/* エラー表示 */}
      {result?.error && (
        <div className="flex items-center gap-2 px-4 py-3 bg-status-danger-bg border border-status-danger/20 rounded-[var(--radius-lg)] text-[13px]">
      ...
      <button
        ...
        disabled={loading}
        className="flex items-center gap-1.5 px-4 py-[7px] bg-accent text-text-inverse text-[13px] font-medium rounded-[var(--radius)] hover:bg-accent-hover transition-colors disabled:opacity-50"
      >
