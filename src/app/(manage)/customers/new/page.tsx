...
export default function NewCustomerPage() {
  ...
  if (done) {
    return (
      <div className="max-w-lg mx-auto">
        <div className="bg-white border border-[#dadce0] rounded-[8px] p-8 text-center">
          <CheckCircle className="w-12 h-12 text-[#188038] mx-auto mb-3" />
          <p className="text-[16px] font-semibold text-[#188038]">顧客を登録しました</p>
          <p className="text-[13px] text-[#5f6368] mt-1">{name}</p>
          <div className="flex gap-2 justify-center mt-6">
            <button onClick={() => { setDone(false); setName(""); setPhone(""); setEmail(""); setRank("regular"); setNotes(""); }}
              className="px-4 py-[7px] border border-[#dadce0] text-[13px] font-medium rounded-[6px] hover:bg-[#f0f1f3]">
              続けて登録
            </button>
            <button onClick={() => router.push("/customers")}
              className="px-4 py-[7px] bg-[#1a73e8] text-white text-[13px] font-medium rounded-[6px] hover:bg-[#1557b0]">
              顧客一覧へ
            </button>
          </div>
        </div>
  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="bg-white border border-[#dadce0] rounded-[8px] p-5">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-[11px] font-semibold text-[#9aa0a6] uppercase tracking-wider mb-1.5">名前 <span className="text-[#c5221f]">*</span></label>
            <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="田中 太郎" className="text-[13px]" required />
          </div>
          <div>
            <label className="block text-[11px] font-semibold text-[#9aa0a6] uppercase tracking-wider mb-1.5">電話番号</label>
            <input type="tel" value={phone} onChange={e => setPhone(e.target.value)} placeholder="090-1234-5678" className="text-[13px]" />
          </div>
          <div>
            <label className="block text-[11px] font-semibold text-[#9aa0a6] uppercase tracking-wider mb-1.5">メールアドレス</label>
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="example@email.com" className="text-[13px]" />
          </div>
          <div>
            <label className="block text-[11px] font-semibold text-[#9aa0a6] uppercase tracking-wider mb-1.5">ランク</label>
          ...
          <div>
            <label className="block text-[11px] font-semibold text-[#9aa0a6] uppercase tracking-wider mb-1.5">メモ</label>
          ...
          <div className="flex gap-2 pt-2">
            <button type="submit" disabled={loading}
              className="flex-1 py-2.5 bg-[#1a73e8] text-white text-[13px] font-medium rounded-[6px] hover:bg-[#1557b0] disabled:opacity-50 transition-colors">
              {loading ? "登録中..." : "登録する"}
            </button>
            <Link href="/customers"
              className="px-4 py-2.5 border border-[#dadce0] text-[13px] font-medium rounded-[6px] hover:bg-[#f0f1f3] text-center transition-colors">
              キャンセル
            </Link>
          </div>
        </form>
