        ...
        <tbody>
          {rules.map((r) => (
            <tr key={r.id} className="border-b border-border-light hover:bg-bg-hover">
              ...
              <td className="px-3 py-2.5">
                <button onClick={() => toggle(r.id)} className={`chip chip-sm ${r.isActive ? "chip-success" : "chip-neutral"}`}>
                  {r.isActive ? "有効" : "停止中"}
                </button>
              </td>
              <td className="px-3 py-2.5">
        ...
        <div className="modal-overlay" onClick={() => { setCreating(false); setEditing(null); }}>
          <div className="modal-card modal-card-lg p-6 space-y-4 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-2 pt-2">
              <button onClick={save} disabled={!draft.name.trim()} className="flex-1 py-2.5 bg-accent text-white text-[13px] font-medium rounded-[var(--radius)] hover:bg-accent-light hover:text-[#157a58] disabled:opacity-50">
                {editing ? "更新" : "作成"}
              </button>
